...
namespace ZBar
{
  ...
  public class Video : IDisposable
  {
    ...
    {
      if (zbar_video_open(_video, device) != 0)
      {
    ...
    public void Close()
    {
      if (zbar_video_open(_video, null) != 0)
      {
        throw new ZBarException(_video);
      }
    }
    ...
    public bool Enabled
    {
      set
      {
        if (zbar_video_enable(_video, value ? 1 : 0) != 0)
        {
          throw new ZBarException(_video);
        }
    ...
    public int Width
    {
      get
      {
        int width = zbar_video_get_width(_video);
    ...
    public int Height
    {
      get
      {
        int height = zbar_video_get_height(_video);
        if (height == 0)
          throw new Exception("Video device not opened!");
        return height;
      }
    ...
    {
      if (zbar_video_request_size(_video, width, height) != 0)
        throw new ZBarException(_video);
    }
    ...
    {
      IntPtr image = zbar_video_next_image(_video);
